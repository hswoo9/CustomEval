	...
	function cancelBtn() {
		customConfirm("기피신청을 확정할 경우, 평가에서 제외되며 다시 참여하실 수 없습니다.\n그래도 진행하시겠습니까?", "warning").then((willConfirm) => {
			if (willConfirm) {
                ...
                html2canvas(document.getElementById("pdf"), { scale: 2 }).then(headerCanvas => {
	                ...
	                formData.append("eval_avoid_txt", $('input:radio[name=avoidReason]:checked').val());

					if ($('input:radio[name=avoidReason]:checked').val() == 'other') {
	                    formData.append("eval_avoid_txt", $('#otherTxt').val());
					}
		...
		<div>
			...
			<div class="com_ta2">
				<div id="pdf" style="padding: 5px;">
					...
					<div id="nameDiv" style="display: none; text-align: right; margin-top: 5px;">
						<span>평가위원명 : ${userInfo.EVAL_USER_ID}</span>
					</div>
				</div>
