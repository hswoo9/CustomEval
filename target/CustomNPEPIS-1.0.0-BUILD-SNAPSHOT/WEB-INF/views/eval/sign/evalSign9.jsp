...
function signSaveBtn(){
	...
	for (var i = 0; i < jsonData.length; i++) {
		...
		_hwpPutText("inputDate{{"+i+"}}", $('input[name="inputDate[]"]')[i].value);
...
// 	var flag = chkData();
	...
	uploader.addParam("step", "9");	//단계
	uploader.addParam("flag", flag);//접촉여부 Y 있다, N 없다
	...
	<tbody>
		<c:forEach var="item" items="${companyList}" varStatus="status">
			...
			<tr>
				...
				<td>
					<input type="text" name="contactor[]" id="contactor${status.count}">
				</td>
				<td>
					<input type="text" name="inputDate[]" id="inputDate${status.count}">
				</td>
				<td>
					<select name="chkData[]" id="chkData${status.count}" style="width: 80px;">
					    <option value="없다" selected="selected">없다</option>
					    <option value="있다">있다</option>
					  </select>
				</td>
				<td>
					<input type="text" name="contents[]" id="contents${status.count}" style="width: 95%;">
				</td>
			</tr>
